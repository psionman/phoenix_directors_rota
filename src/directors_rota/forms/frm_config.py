...
from directors_rota.constants import APP_TITLE, COL_MAXIMUM, TXT_FILE_TYPES
...
class ConfigFrame:
    ...
    def _main_frame(self, master: tk.Frame) -> tk.Frame:
        ...
        button = IconButton(frame, txt.OPEN, "open", self._get_workbook_dir)
        ...
        email_template = ttk.Entry(frame, textvariable=self.email_template)
        email_template.grid(
            row=row, column=1, columnspan=2, sticky=tk.EW, padx=PAD, pady=PAD
        )
        ...
        self._label_and_spinbox(
            frame, row, "Initials column", self.initials_col
        )

        row += 1
        self._label_and_spinbox(frame, row, "Name column", self.name_col)

        row += 1
        self._label_and_spinbox(frame, row, "Email column", self.email_col)

        row += 1
        self._label_and_spinbox(
            frame, row, "Username column", self.username_col
        )

        row += 1
        self._label_and_spinbox(frame, row, "Active column", self.active_col)
        ...
        row += 1
        self._label_and_spinbox(
            frame, row, "Monday date column", self.mon_date_col
        )
        ...
        self.button_frame.grid(
            row=row, column=0, columnspan=4, sticky=tk.EW, pady=PAD
        )
    ...
    def _save_config(self, *args):
        ...
        if result == self.config.STATUS_OK:
            messagebox.showinfo(
                title=APP_TITLE, message="Defaults saved", parent=self.root
            )
        else:
            lf = "\n"
            message = f"Defaults not saved{lf}{result}"
            messagebox.showerror(
                title=APP_TITLE, message=message, parent=self.root
            )
        self._dismiss()
